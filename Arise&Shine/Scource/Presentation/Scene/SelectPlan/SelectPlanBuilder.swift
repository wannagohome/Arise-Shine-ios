...
protocol SelectPlanDependency: Dependency {}

final class SelectPlanComponent: Component<SelectPlanDependency> {}

// MARK: - Builder

protocol SelectPlanBuildable: Buildable {
    func build(withListener listener: SelectPlanListener) -> SelectPlanRouting
}

final class SelectPlanBuilder: Builder<SelectPlanDependency>, SelectPlanBuildable {

    override init(dependency: SelectPlanDependency) {
        super.init(dependency: dependency)
    }

    func build(withListener listener: SelectPlanListener) -> SelectPlanRouting {
        let _ = SelectPlanComponent(dependency: dependency)
        let viewController = SelectPlanViewController.initWithStoryBoard()
        let interactor = SelectPlanInteractor(presenter: viewController,
                                              initialState: SelectPlanPresentableState(schedules: [
                                                Schedule(id: 1, startDate: Date(), schedule: [
        ...
        return SelectPlanRouter(interactor: interactor, viewController: viewController)
    }
}
